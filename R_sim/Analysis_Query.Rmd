...
view_history$viewing_time <- format(view_history$viewing_time, "%Y-%m-%d %H:%M:%S")
```


```{r}
# SQLite에서 문자열 형식의 날짜 데이터를 날짜/시간 타입으로 변환
# 예시: viewing_time 컬럼이 문제의 컬럼이라고 가정
# SELECT strftime('%Y-%m-%d', viewing_time) FROM view_history;
conn = src_memdb()
conn_isolated = conn$con
copy_to(conn, view_history, overwrite = TRUE)
view_history_date = tbl(conn, sql(
"SELECT *
     , strftime('%Y', viewing_time) AS year
     , strftime('%m', viewing_time) AS month
     , strftime('%d', viewing_time) AS _date
     , CASE strftime('%w', viewing_time) 
       ...
       END AS day
 FROM view_history
 ORDER BY viewing_time"))
view_history_date
conn_date = src_memdb()
conn_isolated.date = conn_date$con
copy_to(conn_date, view_history_date, overwrite = TRUE)
```
...
```{r}
boxplot(view_history$viewing_time, view_history$video_link)
```
